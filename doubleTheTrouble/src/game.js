...
function mechanics() {
    ...
    var firstTime = Math.random() * 10000;
    ...
    document.getElementById("start_game").ontouchstart = () => {
        ...
        if(localStorage.getItem(firstTime) == null) {
            ...
            shieldInt = setInterval(spawnShield, 32000);
            coinInt = setInterval(spawnCoin, 1000);
            acidRainInt = setInterval(spawnRain, rainDropRate);
            aestheticInt = setInterval(spawnAestheticRain, 4000);
            trapInt = setInterval(spawnTrap, 4000);
    ...
    document.getElementById("Music").onclick = function () {
        if(localStorage.getItem(tempvar) == 'off' || toggle == null || musicPlay == false) {
 ...
 /*============================================ END SPRITE MECHANICS =================================================================================================================
    ...
    function randomExcludeRange () {
        const randInt = Math.random() * (950-80) + 80;
     if(randInt > 400 && randInt < 600) {
            return randomExcludeRange();
        }else{
            return randInt;
        }
    ...
    function spawnRain() {
        ...
        rainObject = rain.create(randomExcludeRange(), -150, "drop")
    ...
    function spawnAestheticRain () {
        sizes = Math.random() * (200 - 65) + 65;

        fakeRain = rain.create(Math.random() * (570-430) + 430, -150, "drop")
            .setDisplaySize(20, sizes)
            .setAlpha(0.5).setOffset(0, -20);
            console.log('oooooooooo issa fake')
    }
    ...
    function onShieldCollect(sprite, collectedShield) {
        ...
        setTimeout(deactivateShield, 10000);
        ...
        if(isTrapActive === false) {
            ...
            isTrapActive = true;
                ...
                function stopTrap() {
                    ...
                    leftPlatform.setScale(1, 1,);
                    rightPlatform.setScale(1, 1);
                    isTrapActive = false;
                }

                setTimeout(stopTrap, 15000);
    ...
    function onHighScore() {
        ...
        document.getElementById('yourscore').innerHTML = localStorage.getItem(highscoreMem) + '<small>pts</small>';
    }
